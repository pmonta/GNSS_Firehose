...
{ clock_init();
  ...
  set_agc(2,240);
  set_agc(3,240);
//fixme  scratch_uart_phy = 0;        // control of PHY SMI bus is initially local
//fixme  scratch_auto_agc = 1;        // enable automatic AGC by default
//fixme  spi_init_mac(); }            // initialize MAC address from flash
}
...
{ unsigned int j;
  ...
  if (j==jiffies)          // has jiffies counter changed?
    return;
  jiffies = j;
//fixme  agc_service();           // service the AGC loops
//fixme  phy_service(); }         // poll Ethernet PHY for link status
}
